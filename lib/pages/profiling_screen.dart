...
class ProfilingScreen extends StatefulWidget {
  ...
  final Map<String, List<String>> symptomProfile; // SYMPTOMS: LIST<LOCATIONS>
...
class _ProfilingScreenState extends State<ProfilingScreen> {
  List<String> classes = [
    ...
    'vulvovaginitis',
    'vulvovaginitis due to a parasite',
    'vulvovaginitis due to a sexually transmitted disease',
    'vulvovaginitis due to bacteria',
    'vulvovaginitis due to fungus',
    'vulvovaginitis due to protozoa',
    'wasp sting',
    'water intoxication',
    'wheezing',
    'whiplash',
  ...
  Map<String, int> map = {};
  late Map<String, dynamic> highRiskDisease;
  late Map<String, double> doublizedHighRiskDisease;
  late Map<String, double> top5Disease;
  bool _showHighRiskProfile = false;
  bool _isExpanded = false;

  @override
  void initState() {
    Future.delayed(Duration(seconds: 1), () async {
      highRiskDisease = await getDiseaseProba(widget.symptomProfile);
      doublizedHighRiskDisease = convertToDoubleMap(highRiskDisease);
      top5Disease = getTopDiseases(doublizedHighRiskDisease);
      setState(() {
        _showHighRiskProfile = true;
      });
    });
    super.initState();
  }
          ...
          Center(
            child: ProfileVisual(
              patient: Patient(
                name: "Noki",
                age: 20,
                gender: widget.symptomProfile.containsKey("vagina")
                    ? "female"
                    : "male",
              ),
              ...
              ? Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: Color.fromARGB(178, 219, 255, 237),
  ...
  Future<Map<String, dynamic>> getDiseaseProba(
      Map<String, List<String>> symptomProfile) async {
    map = symptoms.asMap().map((index, value) => MapEntry(value, 0));
    print("MAP COUNT: ${map}");

    // encoding input data to i2d in the format of Map<String, int>
    symptomProfile.forEach((key, value) {
      map[key] = 1;
    });

    List<int> inputMatrix = [];

    // constructing the list of input data to i2d
    ...
    var url =
        "https://quecto-i2d-softclassifier-1.eastus.inference.ml.azure.com/score";

    var apiKey = "UcWobz04EOCu5stamaQeUsM6zSW8ohJZ";
    var headers = {
      "Content-Type": "application/json",
      "Authorization": "Bearer ${apiKey}",
      "azureml-model-deployment": "i2d-soft-classifier-2"
    };
    var body = jsonEncode({
      "data": [inputMatrix]
    });
    try {
      ...
      print("encoded entity: ${body}");

      if (response.statusCode == 200) {
        print('Predict Disease Model Response: ${response.body}');
      } else {
        print(
            'Predict Disease Request failed with status code ${response.statusCode}');
        print('Predict Disease Model Response: ${response.body}');
      }
  ...
  Map<String, double> getTopDiseases(Map<String, double> diseaseProba) {
    List<MapEntry<String, double>> sortedEntries = diseaseProba.entries.toList()
      ..sort((a, b) => b.value.compareTo(a.value));

    List<MapEntry<String, double>> top5Entries = sortedEntries.take(5).toList();

    return Map.fromEntries(top5Entries);
  }

  Map<String, double> convertToDoubleMap(Map<String, dynamic> originalMap) {
